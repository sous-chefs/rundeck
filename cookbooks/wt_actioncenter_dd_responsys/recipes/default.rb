...
if node.recipes.include?("wt_actioncenter_ddp") then
	...
	log "running on DDP box, cleaning up proxy_uri to go directly to a partner's endpoint"
	proxy_uri = ""
else 
	log "running on non-DDP box, must go thru a proxy"
end
...
%w[config.properties responsys.conf ResponsysServerCertificate.cer].each do | template_file|
